    ...
    async def leave_team(self, ctx):
        ...
        for team_name, team in teams.items():
            ...
            if f"{ctx.author.id}" in team["players"]:
                ...
                break
        if player_kicked:
            await ctx.send(f'{ctx.author.mention} has departed from their team.')
        else:
    ...
    async def player_info(self, ctx, player: discord.Member):
        ...
        for t_name, team in teams.items():
            if f"{player.id}" in team["subplayers"]:
                # Create an embed with the player's information
                embed = discord.Embed(title=player.display_name, color=16773632)
                embed.add_field(name="Tracker", value=f"[Click here](team['subplayers'][f'{player.id}']['tracker'])", inline=False)
